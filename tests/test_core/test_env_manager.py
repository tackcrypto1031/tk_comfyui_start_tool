...
class TestCloneEnvironment:
    ...
    def test_clone_basic(self, mock_snap_cls, mock_copytree, sample_config):
        ...
        mock_copytree.side_effect = lambda src, dst, **kw: Path(dst).mkdir(parents=True, exist_ok=True)
    ...
    def test_clone_copies_directory(self, mock_snap_cls, mock_copytree, sample_config):
        """Clone copies the entire source directory, excluding runtime artifacts."""
        _create_mock_env(sample_config["environments_dir"], "main")
        mock_snap_cls.return_value.create_snapshot.return_value = MagicMock()
        mock_copytree.side_effect = lambda src, dst, **kw: Path(dst).mkdir(parents=True, exist_ok=True)
        ...
        assert call_args.args == (source_dir, target_dir), "copytree called with wrong positional args"
        assert "ignore" in call_args.kwargs, "copytree must receive an 'ignore' kwarg to exclude runtime artifacts"
...
def test_toggle_shared_model_builds_junctions_on_windows(tmp_path):
    from src.core.env_manager import EnvManager
    ...
    config = {
        ...
        "custom_model_path": "",
        "model_subdirs": ["checkpoints", "configs"],
    ...
    (env_dir / "ComfyUI/models/configs").mkdir(parents=True)
    (env_dir / "env_meta.json").write_text(
        '{"name":"test","created_at":"2026-04-20T00:00:00","shared_model_enabled":false}',
        encoding="utf-8",
    )

    mgr.toggle_shared_model("test", True)

    from src.utils import fs_ops
    assert fs_ops.is_junction(env_dir / "ComfyUI/models/checkpoints")
    assert not fs_ops.is_junction(env_dir / "ComfyUI/models/configs")


@pytest.mark.skipif(platform.system() != "Windows", reason="junction test")
def test_remove_environment_does_not_wipe_shared_models(tmp_path):
    from src.core.env_manager import EnvManager
